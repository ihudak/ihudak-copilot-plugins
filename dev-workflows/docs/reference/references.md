...
`dev-workflows` bundles 97 files under `skills/_shared/` — 37 top-level markdown files and six vendored subtrees. This page enumerates every one of the 37 top-level files by name, grouped by concern below, then counts the six subtrees rather than listing each file inside them. The arithmetic: 37 named individually, plus 24 + 11 + 10 + 6 + 3 + 2 = 56 markdown pages counted (not enumerated) across the six subtrees — 37 + 56 = 93 markdown files, against 97 files on disk. The remaining four are non-markdown vendored data inside those same subtrees, deliberately not listed as reference pages: `api-guidelines/template/openapi-template.yaml`, `dynatrace-docs/docs-profile.default.yml`, `dynatrace-docs/managed-owners.txt`, and `guidelines/check_guidelines.py` — a template, a defaults file, an owners list, and a lint script, none of them prose a reader would open. The 56-file subtree figures below are markdown-page counts specifically; the four files above already sit inside those same subtrees and are not part of that count, so nobody should later "correct" a subtree figure by adding them back in. Unlike the Claude edition, this edition has no `references/model-routing/` subtree and no cost-prices.yaml file — `model-routing.md` is a flat file directly under `skills/_shared/`, counted among the 37, and there is no cost-price file to count because this edition has no cost subsystem at all (`skills/_shared/specs-repo-git.md:54`).
...
- `grilling-technique.md` — the interview technique every authoring skill (and `prompt-grill-me:`) uses to refine an artifact; embedded so callers carry no runtime dependency. Its rhythm follows the caller's depth: bounded callers ask one question at a time, because a cap counted in questions is only enforceable that way; relentless callers work the design tree's settled frontier one numbered round at a time. Both end at a confirmation gate the user closes, never the agent.
...
The git entry points that bound every write into the specs repo and into a code repo, plus the naming and read-only-mount conventions those entry points depend on.
...
- `code-repo-handoff.md` — the code repo's own finish: commit what the run wrote onto the branch it created, push it, and open a pull request where the host allows one. The commit is prompt-free because the work is not safe until it exists; the push and the pull request sit behind one consent choice, asked once per run. Consumed by `implement:`, `vuln:`, and `upgrade:`.
...
This edition bundles twenty-one `skills/` entries in total, all documented under [Skills](../README.md#skills). Twenty of those are the pipeline/command skills, of which only fourteen are *pipeline* skills that load `model-routing.md` (see below); the other six (`api-guideline-reviewer:`, `guideline-reviewer:`, `feedback:`, `prompt:`, `prompt-brainstorm:`, `prompt-grill-me:`) are exempt — each of those **is** a Copilot CLI skill (a SKILL.md file with `name:`/`description:` frontmatter), so there is no separate "N bundled skills" count for that group. `model-routing.md` is a flat reference file consumed by path, not an invocable skill.

The twenty-first entry is `dynatrace-docs-frontmatter` — referenced by `docs-profile:`, `document:`, and the `changelog-owners-reminder` hook as "the `dynatrace-docs-frontmatter` skill," and, unlike the fourteen and six above, neither a pipeline step nor an exempt-but-ordinary command: it applies frontmatter conventions while another skill (or the user) edits a dynatrace-docs page, rather than owning a workflow step of its own. It is bundled here at `skills/dynatrace-docs-frontmatter/SKILL.md`, ported from the canonical and mgd editions of this plugin, which bundle the same skill; its underlying guideline files were already bundled here too, at `skills/_shared/dynatrace-docs/changelog-guidelines.md` and `skills/_shared/dynatrace-docs/frontmatter-guidelines.md` — only the skill wrapper itself was missing before this port. See [`dynatrace-docs-frontmatter:`](../skills/dynatrace-docs-frontmatter.md) for its own page.
