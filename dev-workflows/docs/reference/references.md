...
Unlike the Claude edition, this edition bundles no `skills/` subtree of its own beyond the twenty skills already documented under [Skills](../README.md#skills) — only fourteen of those twenty are *pipeline* skills that load `model-routing.md` (see below); the other six (`api-guideline-reviewer:`, `guideline-reviewer:`, `feedback:`, `prompt:`, `prompt-brainstorm:`, `prompt-grill-me:`) are exempt — each of those **is** a Copilot CLI skill (a SKILL.md file with `name:`/`description:` frontmatter), so there is no separate "N bundled skills" count here. `model-routing.md` is a flat reference file consumed by path, not an invocable skill. The one skill named throughout this plugin's source that genuinely isn't bundled here is `dynatrace-docs-frontmatter` — referenced by `docs-profile:`, `document:`, and the `changelog-owners-reminder` hook as "the `dynatrace-docs-frontmatter` skill." It is not a dependency on something external: the canonical and mgd editions of this plugin both bundle it, as `skills/dynatrace-docs-frontmatter/SKILL.md`, and its underlying guideline files are bundled here too, at `skills/_shared/dynatrace-docs/changelog-guidelines.md` and `skills/_shared/dynatrace-docs/frontmatter-guidelines.md` — only the skill wrapper itself was never ported to this edition. That is a known gap in this port, tracked here rather than silently worked around, which is why there is no page for it in this reference section.
