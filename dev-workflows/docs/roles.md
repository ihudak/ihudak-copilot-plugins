...
- **Produces:** `design.md`, landed on the specs repo's default branch; code committed on a feature branch in `$REPOS_PATH` — pushed, with a pull request opened, once you approve that half; product documentation in the external docs repo; the final release-notes draft; and, from `ready:`, a `SUPPORTED` / `PARTIAL` / `NOT-SUPPORTED` verdict plus an optional `_readiness.md` snapshot, committed and handed off only behind your consent.
