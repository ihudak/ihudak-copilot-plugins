...
- [Roles](../roles.md) — the Dev role (`design:`, `implement:`, `document:`, `ready:`); `document:` (Mode A) is the heaviest of the four, by `## Phase` heading count (25, against design:'s 12, implement:'s 16, and ready:'s 10).
- [`epics:`](epics.md) — the sibling skill for writing child Epic drafts instead of product documentation.
- [`release-notes:`](release-notes.md) — the downstream skill for the customer-facing announcement, once every Epic under the VI is documented.
- [`docs-profile:`](docs-profile.md) — the skill Phase 0 runs inline, on demand, for a custom docs repo with no profile yet.
