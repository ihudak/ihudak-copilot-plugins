...
1. **Decide what topics the page must cover.** Typical topics, sourced from the VI goal + Epic summaries + diff summaries:
   ...
   - (Do NOT add a "What's new" / release-notes topic here — a page whose frontmatter carries `meta.content-type: release-notes`, or anything under `_data/release-notes/…` / `_snippets/release-notes/…`, is generated by Jira-driven automation, not authored in the repo. Release notes are produced by the separate `release-notes:` command. **A page declared in `profile.announcement_pages` is exempt** — announcement pages are hand-authored and nothing generates them.)
...
3. **Plan frontmatter updates** (field rules: `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/dynatrace-docs/frontmatter-guidelines.md`; changelog + owners keep their own references).
   - `changelog:` — append a dated entry with a customer-readable 1-line change summary and NO Jira key. Create the field if it doesn't exist on an extended page. This is mandatory on every target. Per `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/doc-structure-conventions.md` §1 (verified against the repo convention — fewer than 5 of dynatrace-docs's 5500+ entries cite an issue key).
...
component_patterns:              # recurring content shapes observed in the step-5 sibling sample; [] when the sample shows no established pattern — see ~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/doc-structure-conventions.md §3
