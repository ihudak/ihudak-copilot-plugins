...
- **`existing_image_decisions`** — the orchestrator's Phase 5.6 stale-image-swap array, each `{target, occurrence, old_url, new_url, section, gating, decision}`; `[]` when no existing-image review occurred. Needed for the Screenshots dimension's swap-completeness check — a reviewer cannot check a dimension it was never handed.
...
| Page structure conventions | A callout sits adjacent to what it qualifies — with the option it describes when the option is one of a mutually exclusive set, in the lead-in when it applies to the whole set, never as a trailing block after the set — per `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/doc-structure-conventions.md` §2. A scope violation is **MAJOR** (a misread scope changes what the customer believes is required or prohibited). A content shape with an established sibling pattern in the checklist's `component_patterns` reuses that pattern instead of an ad-hoc structure, per §3; divergence is **MINOR** (an ad-hoc structure still renders). Skip the component-pattern half with "N/A — no `component_patterns`" when that block is empty; the callout-scope half always applies. |
