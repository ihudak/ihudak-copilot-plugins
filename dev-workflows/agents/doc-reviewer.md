...
- **`doc-planner` checklist** — the full YAML checklist from Phase 5.7 (review against plan), including the planner's `repo_authoring_guidance` block (the repo-specific authoring rules to check adherence against), its `repo_verification_gates` block (the repo's own pre-PR checks), and its `component_patterns` block (recurring content-shape → dominant-component evidence, per `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/doc-structure-conventions.md` §3) — needed for the Page structure conventions dimension's component-fidelity half.
...
| Structural integrity | Headings use the repo's expected hierarchy; internal links (`[[wikilinks]]` and `[text](relative-path)`) resolve to existing files; sidebar / index / nav pages (if the repo uses them) are updated. Anchor form and the `validate-anchors` contract also apply, per `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/dynatrace-docs/anchor-conventions.md`: a heading carries at most one `{:#id}`; an anchor link targets a hardcoded id. |
| Page structure conventions | A callout sits adjacent to what it qualifies — with the option it describes when the option is one of a mutually exclusive set, in the lead-in when it applies to the whole set, never as an **unqualified** trailing block after the set — per `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/doc-structure-conventions.md` §2. §2 rule 3 is an explicit alternative, not an exception to be argued: a callout trailing **the set it qualifies** and naming its own scope in its first clause (e.g. *"This applies only to the built-in cluster registry."*) is compliant and is **not** a finding — rule 3 licenses that placement only, so a scope-labelled callout sitting away from its set (several sections later, or under an unrelated option) is still a finding. A scope violation is **MAJOR** (a misread scope changes what the customer believes is required or prohibited). A content shape with an established sibling pattern in the checklist's `component_patterns` reuses that pattern instead of an ad-hoc structure, per §3; divergence is **MINOR** (an ad-hoc structure still renders). Skip the component-pattern half with "N/A — no `component_patterns`" when that block is empty; the callout-scope half always applies. |
...
| Screenshots | For `image_policy: local` targets, every referenced image file resolves on disk. For `image_policy: cdn_upload_required` targets, a TODO placeholder is present in the markdown AND the Phase 9 `### Screenshots to upload manually` section lists the staged file (the reviewer checks the TODO is present; the Phase 9 section content is the command's responsibility). Alt-text is present in all cases. Swap completeness: every `existing_image_decisions` entry with `decision: accepted` has its `old_url` replaced by its `new_url` at that exact `occurrence` in `target` — a missed swap leaves a stale image live and is invisible in the diff, **MAJOR**. Every `decision: declined` entry is left untouched — a swap applied there is equally **MAJOR**, since it can push a replacement into a space the change does not affect. |
...
| Source traceability | Per `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/doc-structure-conventions.md` §1, the rendered page carries no source provenance: a Jira key — bare, or as a `[[<JIRA_KEY>]]` wikilink — a PR URL, or a `<!-- KEY: … -->` HTML comment appearing anywhere in a written file is **MAJOR**. An internal `[[wikilink]]` whose target is another docs page, not a Jira key, is dimension 5's business, not this one's — a correctly-resolving cross-reference is not a traceability finding. A valid `<!-- intentional-discrepancy: … -->` marker (`~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/source-truth.md` §7.6) is the one permitted comment and is not a finding. |
...
- NEVER raise a BLOCKER on a claim that carries a valid `<!-- intentional-discrepancy ... -->` marker — it is a user-acknowledged gap (see `~/.copilot/installed-plugins/ihudak-copilot-plugins/dev-workflows/skills/_shared/source-truth.md` §7.6). Note it as a recorded gap instead.
