...
| `design:` | design | Write an engineering design from a specification. On a contested interface, offers a three-take `interface-designer` fan-out feeding `design.md`'s `### Alternatives considered` section; `--design-twice` forces the offer even with no signal fired. Reviewed by `design-reviewer`. |
