...
| `design:` | design | Write an engineering design from a specification. On a contested interface, offers a three-take `interface-designer` fan-out feeding `design.md`'s `### Alternatives considered` section; `--design-twice` forces that fan-out with no offer, even with no signal fired. Reviewed by `design-reviewer`. |
...
|------|-------------|----------|---------------------------|
| **PM** | `idea:`, `create-vi: <KEY>`, `update-vi: <KEY>`, `release-notes: <VI>` | a prompt / community post / RFE / existing VI; then a refined `idea.md` + a JIRA-KEY | `<KEY>_<slug>.md` in `$SPECS_PATH/specifications/<KEY>-<slug>/` (`idea:` relocates `idea.md` there itself, once a key resolves — `create-vi:` finds it in place and never moves it); an early release-notes draft in the vault; paste-to-Jira → re-import to `$VAULT_PATH/jira-products/<KEY>/` |
...
flowchart TD
    ...
    G -->|Yes| RV["test-writer → strong-tier code-review → triage (verify each finding) → review-fixer (gate: tests never run before non-BLOCK)"]
...
| `code-review` | Strong tier, caller-pinned | Post-implementation reviewer — 11 dimensions: eight always (correctness, security, architecture, edge cases, migration, dependencies, test adequacy, rollback) plus three conditional (ARD conformance, spec/design conformance, claims falsification). Verdict: PASS / PASS WITH RECOMMENDATIONS / BLOCK. BLOCK gates the test run. |
| `doc-reviewer` | Strong tier, caller-pinned | Product-documentation reviewer for `document:` — 18 dimensions including factual correctness, completeness vs plan, audience fit, structural integrity (incl. anchor form), page structure conventions (callout scope + component-pattern fidelity), YAML frontmatter, screenshots (incl. stale-image swap completeness), snippets, actionability, source traceability (a Jira key or PR URL in a rendered page is a MAJOR — the rendered page carries no source provenance), cross-space grounding integrity, style-check follow-through, and claims falsification (the fixer's account of its own work, read only after every other dimension). |
| `epic-reviewer` | Strong tier, caller-pinned | Epic-draft reviewer for `epics:` — goal clarity, testable acceptance criteria, scope boundaries, dependencies, non-duplication vs sibling Epics (BLOCKER), reference-path evidence (when `code-scanner` output is provided), and claims falsification (the fixer's account of its own work, read only after every other dimension). |
...
| `review-fixer` | Caller-assigned (default, not strong tier) | Applies BLOCKER / MAJOR findings from a `code-review` report. Receives **survivors only** — the orchestrator has already triaged the findings (`_shared/finding-triage.md`), so every finding that arrives is one whose claimed consequence was verified; it does not re-triage and its dispositions stay Applied / Deferred. Applies that reference's **patch gate**: the fix must add no public surface and guard no state the finding did not demonstrate — a survivor whose smallest correct fix would do either is deferred as `DEFERRED — needs human decision` rather than patched speculatively. Returns a structured fix report. Used by `implement:`, `vuln:`, `upgrade:`. |
...
| `doc-fixer` | Caller-assigned | Applies BLOCKER / MAJOR findings from a `doc-reviewer`, `epic-reviewer`, or `docs-style-checker` report. Shared between `document:` and `epics:`. From a **reviewer** the list arrives already triaged (`_shared/finding-triage.md`) — **survivors only**, never re-triaged; from a **style checker** no triage ran, because a linter violation is not a claim about consequence. Applies that reference's **patch gate**: no section, heading, page, cross-reference, or content beyond what the finding names — anything more is deferred as `DEFERRED — needs human decision`. |
...
| `interface-designer` | Caller-assigned | Produces ONE interface proposal for ONE contested interface under ONE named constraint (minimise the interface / maximise flexibility / optimise for the most common caller), for `design:`'s three-take fan-out — dispatched three times in parallel, each blind to the others, so the takes diverge. Read-only; proposes an interface, never writes one. Used by `design:` only. |
...
- **Follow-up task emission** (`document:`, `release-notes:`, `epics:`, `implement:`, `ready:`) persists out-of-scope / manual-step follow-ups as durable Obsidian tasks via a batch preview. Works without the `obsidian-llm-wiki` plugin (mirrors its task conventions internally). Degrades gracefully: `$VAULT_PATH` → the VI's `$SPECS_PATH` dir → beside the imported Jira directory → report-only. See [`skills/_shared/followup-emission.md`](skills/_shared/followup-emission.md).
- **Specs-repo git completeness (all seventeen skills that write to `$SPECS_PATH`).** The specs repo maintains itself. At run start — as early as `$SPECS_PATH` is known (Phase 0 in most skills, Step 0 in `vuln:`, the shared mode-detection section in `document:`) — `specs-preflight` commits and pushes any artifacts a previous run left behind, retries a commit whose push failed, and settles the branch — switching away only from branches the plugin created, and standing still on anything else. As the run's last action — or, where a later phase cedes control to another skill or a long interactive stretch, immediately before that hand-off — `commit-artifacts` stages the bounded artifact paths, commits `<KEY|NOISSUE> Add dev-workflows session artifacts (<skill>)`, and pushes; for the eight producing skills that opened a specs-repo PR at handoff via `handoff-to-main` (`idea:`, `create-vi:`, `update-vi:`, `create-ard:`, `specify:`, `design:`, `implement:`, `ready:`), that push updates the PR they already opened. It is bounded to two path shapes inside `$SPECS_PATH`, never issues `git add -A` at repository scope, never force-pushes, never deletes a branch with `-D` or a lock file, and never fails the run. A detached HEAD blocks the commit outright and says so loudly — a commit made there would be unreachable and garbage-collectable, and reporting a SHA over it would be a failure that looked like success. See [`skills/_shared/specs-repo-git.md`](skills/_shared/specs-repo-git.md).
...
- `ard-resolution.md` — most-specific-first ARD resolution (per-area → Epic-level → inherited VI-level) consumed by `create-ard:`, `design:`, `implement:`, `specify:`, `epics:`, and `ready:`
...
- `grilling-technique.md` — the embedded bounded one-question-at-a-time grilling SSOT (used by `idea:`, `create-vi:`, `update-vi:`, `create-ard:`, `specify:`, `design:`, `prompt-grill-me:`)
...
- `context-management.md` — mid-run context-window guidance (checkpoint → offload → decompose, and "hand off by file, not paste" with the `mktemp`-outside-every-repo-tree guard) plus **the read-failure contract** for every input a caller may hand over inline or as a path: an unreadable **evidence** input is a hard stop that is never regenerated by other means, an unreadable **context** input degrades to absent and the output records the degradation, and the tier is fixed by the consuming agent where it takes that input, rather than decided at runtime. Cited by `implement:`, `vuln:`, and `upgrade:`, and stated by `risk-planner`, `code-review`, `test-writer`, `review-fixer`, `vuln-fixer`, and `upgrade-executor`
- `finding-triage.md` — the orchestrator-run step between an Opus/strong-tier reviewer's findings and a fixer's edits: verify each finding's own claimed consequence at the location it names, keep or dismiss, record every dismissal with a reason that disposes of that finding's own claim, and hand the fixer survivors only. Carries the patch gate (never a fix that guards state the finding did not demonstrate) and the reporting contract (a triage that reports only survivors is indistinguishable from a reviewer that found less). Attaches wherever a reasoned-claim producer feeds a fixer — `code-review` → `review-fixer` (`implement:`, `vuln:`, `upgrade:`), `doc-reviewer` → `doc-fixer` (`document:` Jira mode), `epic-reviewer` → `doc-fixer` (`epics:`) — and never to a style-checker-fed dispatch
- `instruction-file-maintenance.md` — rules for changing an agent-instruction file (`.github/copilot-instructions.md`, rules files, and this plugin's own `_shared/*.md`): verify every command claim against the thing that runs it, itemise a narrowing rewrite as the deletion it is, give a pointer an observable trigger rather than one the agent must judge, treat two live contradictory instructions as a defect, and require grounds for retirement — never "it looks derivable", never "nothing has failed on it lately". Consulted by `impl-maintenance` when it proposes changes, and binding on hand edits — which is where most stale claims originate
...
- `release-note-types.md` — the release-note destination map (breaking-changes / feature-updates / fixes), the per-destination draft shape and prose rules, the deprecation-note rule, and Change Type sourcing, for `release-notes:`
...
- `followup-emission.md` — the end-of-run follow-up task emitter shared by `document:`, `release-notes:`, `epics:`, `implement:`, and `ready:`
- `specs-repo-git.md` — the two specs-repo git entry points shared by all seventeen skills that write into `$SPECS_PATH`: `specs-preflight` (run start — flush leftover artifacts, retry an unpushed artifact commit, settle the branch) and `commit-artifacts` (terminal — stage the bounded artifact paths, commit, push). Owns the bounded write authority (two path shapes; `^(idea|vi|ard|spec|design|ready)/` branches only — the six-prefix authority), the three guards and their four-part notice contract, the branch-disposition table, and the `Specs repo:` outcome line. Always `git -C "$SPECS_PATH"`, never a `cd`; never force-pushes; never fails the run. Bookkeeping only — a phase's own deliverable is `phase-handoff.md`'s to commit and hand off
- `phase-handoff.md` — the two phase-boundary git entry points that move a phase **deliverable** (as opposed to bookkeeping) onto `$SPECS_PATH`'s default branch: `handoff-to-main` (§2, producer — resolves or reuses a plugin branch, stages the deliverable by enumeration, commits with a carried `Co-authored-by` trailer, pushes, and opens a pull request via a `gh` capability probe) and `require-on-main` (§3, consumer — a ten-state gate, rows H/I/G/A/B/C′/C/D/E/F, tested against `origin/<default>` by ref rather than by worktree contents; §3.4's row-F delegation table means an absent gated input always falls back to the caller's pre-existing behaviour, never a new prerequisite). Inherits four of `specs-repo-git.md`'s hard rules unchanged and deliberately diverges on three (§1 rules 5–7): `require-on-main` is fatal by design, its commit carries a `Co-authored-by` trailer, and `handoff-to-main` opens a pull request — all only behind the caller's own §4.3 consent choice. Eight producers call `handoff-to-main` (`idea:`, `create-vi:`, `update-vi:`, `create-ard:`, `specify:`, `design:`, `implement:`, `ready:`); seven consumers call `require-on-main` (`create-vi:`, `create-ard:`, `specify:`, `design:`, `implement:`, `epics:`, `ready:` — `update-vi:` deliberately calls neither, since its base is the Jira import)
