...
| `code-scanner` | Caller-assigned | Scans one repo for existing capabilities and gaps relative to themes (from an Epic, an implementation spec, or an idea's themes). Fanned out one-per-repo, cap 4 concurrent; evidence entries may carry an optional `lines` array when the match came from a grep hit. Used by `epics:`, `implement:` (multi-source fan-out), and `idea:` (`--ground-code`, broad-then-narrow per `_shared/model-routing.md` §8.5). |
